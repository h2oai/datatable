...
template<typename T>
void assert_compatible_type(SType s) {
  throw NotImplError() << "Invalid type T in assert_compatible_type()";
}

template<>
void assert_compatible_type<int32_t>(SType s) {
  xassert(s == SType::BOOL ||
          s == SType::INT8 || s == SType::INT16 || s == SType::INT32);
}

template<>
void assert_compatible_type<int64_t>(SType s) {
  xassert(s == SType::INT64);
}

template<>
void assert_compatible_type<float>(SType s) {
  xassert(s == SType::FLOAT32);
}

template<>
void assert_compatible_type<double>(SType s) {
  xassert(s == SType::FLOAT64);
}

template<>
void assert_compatible_type<CString>(SType s) {
  xassert(s == SType::STR32 || s == SType::STR64);
}

template<>
void assert_compatible_type<py::robj>(SType s) {
  xassert(s == SType::OBJ);
}
