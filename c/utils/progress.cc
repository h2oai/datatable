...
void run_interleaved(size_t nrows, void (*run)(size_t, size_t, size_t))
{
  ...
  size_t nth = std::min(static_cast<size_t>(config::nthreads),
  ...
  if (nth <= 1) {
    ...
    #pragma omp parallel nthreads(nth)
    {
      try {
        size_t ith = static_cast<size_t>(omp_get_thread_num());
        size_t nth = static_cast<size_t>(omp_gen_num_threads());
        if (ith == 0) {
