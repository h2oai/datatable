...
columns_from_slice(DataTable *dt, int64_t start, int64_t count, int64_t step)
{
    ...
    for (int64_t i = 0, j = start; i < count; i++, j += step) {
        columns[i] = srccols[j] == NULL ? NULL : srccols[j]->incref();
    }
    return columns;
}
...
columns_from_array(DataTable *dt, int64_t *indices, int64_t ncols)
{
    ...
    for (int64_t i = 0; i < ncols; i++) {
        columns[i] = srccols[indices[i]] == NULL ? NULL : srccols[indices[i]]->incref();
    }
    return columns;
}
...
Column** columns_from_mixed(
    ...
    for (int64_t i = 0; i < ncols; i++) {
        if (spec[i] >= 0) {
            columns[i] = dt->columns[spec[i]] == NULL ? NULL : dt->columns[spec[i]]->incref();
        } else {
