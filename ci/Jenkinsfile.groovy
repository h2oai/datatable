...
"""
                                    ...
                                    sh """
                                        ...
                                        cd release
                                        hub release create -f ${
                                        releaseMsgFile
                                    } \$(find dist/ \\( -name '*.whl' -o -name '*.tar.gz' \\) -exec echo -a {} \\;) "v${
                                        versionText
                                    }"
                                    """
...
def testInDocker(final testTarget, final needsLargerTest) {
    try {
        ...
        junit testResults: "build/test-reports/TEST-*.xml", arch "build/test-reports/TEST-*.xml", keepLongStdio: true,
