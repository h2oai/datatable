...
.. changelog::
  ...
  -[new] :meth:`Frame.copy()` can now be used to create a copy of the Frame.

  -[enh] :meth:`Frame.cbind()` now accepts a list of frames as the argument.

  -[enh] Frame can now be sorted by multiple columns.

  -[new] Implemented :meth:`Frame.replace()` function. [#1319]
  ...
  -[api] Method :meth:`Frame.rename()` removed -- the :attr:`.name` setter can
    be used instead.

  -[api] :meth:`Frame.resize()` removed -- same functionality is available via
    assigning to :attr:`Frame.nrows`.

  -[api] The expression ``Frame([])`` now creates a ``[0 x 0]`` Frame instead of
   ``[0 x 1]``.

  -[api] Parameter ``inplace`` in :meth:`Frame.cbind()` removed (was
    deprecated). Instead of ``inplace=False`` use ``dt.cbind(...)``.

  -[api] :meth:`Frame.cbind()` no longer returns anything (previously it
    returned self, but this was confusing w.r.t whether it modifies the target,
    or returns a modified copy).

  -[api] default format for :meth:`Frame.save()` is now "jay".
  ...
  -[fix] :meth:`Frame.sort()` with no arguments no longer produces an error.
  ...
  -[new] Added function :func:`.abs()` to find the absolute value of elements
    in a frame.
