...
.. changelog::
    ...
    -[new] Class :class:`dt.FExpr` now has methods :meth:`.sd() <dt.FExpr.sd>`,
      ...
      and :meth:`.last() <dt.FExpr.last>`, :meth:`.shift() <dt.FExpr.last>`,
      which behaves exactly as the equivalent base level functions :func:`dt.sd()`,
    ...
    -[fix] Fread no longer detects separators within quoted strings. [#922]
    ...
    -[api] Removed function ``dt.open()``, which was deprecated before. [#3018]
    ...
    -[new] Added stats functions ``.skew()`` and ``.kurt()``. [#1084]
