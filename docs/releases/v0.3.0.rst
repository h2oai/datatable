...
.. changelog::
  ...
  -[enh] Method :meth:`Frame.tonumpy()` now has argument ``stype`` which will
    force conversion into a numpy array of the specific stype.
  ...
  -[enh] :func:`.isna()` function now works on strings too.

  -[enh] :func:`.save()` is now a method of ``Frame`` class.
  ...
  -[new] New methods :func:`.min1()`, :func:`.max1()`, :func:`.mean1()`,
    :func:`.sum1()`, :func:`.sd1()`, :func:`.countna1()` that are similar to
    :func:`.min()`, :func:`.max()`, etc. but return a scalar instead of a Frame
    (however they only work with a 1-column Frames).

  -[new] Implemented method :func:`.nunique()` to compute the number of unique
    values in each column.

  -[new] Added stats functions :func:`.mode()` and :func:`.nmodal()`.
  ...
  -[api] Parameter ``inplace`` of method :meth:`rbind()` was removed: instead
