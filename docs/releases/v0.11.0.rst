...
.. changelog::
  ...
  Frame
  -----
  .. xpy:current-class:: datatable.Frame

  -[new] Property :xpy:attr:`.source` contains the name of the file
  ...
  -[new] The expression :xpy:meth:`len(DT) <datatable.Frame.__len__>` now works,
  ...
  -[new] Added a new function :xpy:func:`datatable.cut` to bin numeric data
    to equal-width discrete intervals. [#2483]

  -[new] Added a new function :xpy:func:`datatable.qcut` to bin data
    to equal-population discrete intervals. [#1680]

  -[new] Added function :xpy:func:`datatable.math.round` which is the
    equivalent of Python's built-in ``round()``. [#2285]

  -[enh] Method :xpy:meth:`.colindex()` now accepts a column selector
  ...
  -[enh] Added new argument ``bom=False`` to the :xpy:meth:`.to_csv()` method.
  ...
  -[api] Method :xpy:meth:`.cbind()` now throws a
    :xpy:exc:`dt.exceptions.InvalidOperationError`
    instead of a ``ValueError`` if the argument frames have incompatible
    shapes.

  -[api] Method :xpy:meth:`.colindex()` now throws an :xpy:exc:`dt.exceptions.KeyError` when given
    a column that doesn't exist in the frame, or an :xpy:exc:`dt.exceptions.IndexError` if
  ...
  -[api] Parameters ``_strategy=`` in :xpy:meth:`.to_csv()` and :xpy:meth:`.to_jay()`
    were renamed into ``method=``. The old parameter name still works, so this
    change is not breaking.

  -[api] The behaviour of a method :xpy:meth:`.sort()` is made consistent with
    the function :xpy:func:`dt.sort()`. When the list of columns to sort
  ...
  -[fix] It is no longer allowed to combine ``compression="gzip"`` and
    ``append=True`` in :xpy:meth:`.to_csv()`.

  -[fix] Empty strings no longer get confused with NA strings in
    :xpy:meth:`.replace()`. [#2502]

  -[fix] :xpy:func:`dt.rbind()`-ing an iterator of frames created on-the-fly
    no longer produces an undefined behavior. [#2621]


  Fread
  -----
  .. xpy:current-module:: datatable

  -[new] Added new function :xpy:func:`iread()`, which is similar to
    :xpy:func:`fread()`, but suitable for reading multiple sources at once. The
    ...
    The function :xpy:func:`iread()` has parameter ``errors=`` which controls
  ...
  -[api] Function :xpy:func:`fread()` now always returns a single Frame object;
    previously it could return a dict of Frames if multiple sources were
    detected. Use :xpy:func:`iread()` if you need to read multi-source input.
  ...
  -[enh] Added 2 new fields into the :xpy:data:`dt.build_info` struct: ``.git_date``
  ...
  -[fix] Internal function :xpy:func:`dt.internal.frame_column_data_r`
  ...
  -[fix] Fixed possible crash in :xpy:func:`rbind()` and :xpy:func:`union()` when
  ...
  -[api] All exceptions thrown by datatable are now declared in the
    :xpy:mod:`datatable.exceptions` module. These exceptions are now organized to
    derive from the common base class :xpy:exc:`dt.exceptions.DtException`.
  ...
  FTRL model
  ----------
  .. xpy:current-class:: datatable.models.Ftrl

  -[enh] :xpy:attr:`.nepochs`, the number of epochs to train the model, can now
    be a float rather than an integer.

  -[enh] :xpy:meth:`.fit()` now throws :xpy:exc:`dt.exceptions.TypeError` when ltypes in the
   training and validation frames are not consistent.

  -[api] :xpy:attr:`.interactions` now throws an :xpy:exc:`dt.exceptions.ValueError`
    instead of a :xpy:exc:`dt.exceptions.TypeError` when assigning interactions
