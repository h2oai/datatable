#!/usr/bin/env python3
# Copyright 2017 H2O.ai; Apache License Version 2.0;  -*- encoding: utf-8 -*-
#
# See http://pytest-benchmark.readthedocs.io/en/stable
#
import datatable
import pytest


#-------------------------------------------------------------------------------
# Sort booleans
#-------------------------------------------------------------------------------

@pytest.fixture(params=[10**4, 10**6, 10**8])
def ndarray_bool(numpy, benchmark, request):
    n = request.param
    benchmark.group = "sort(dtype=bool, n=%d)" % n
    a = numpy.random.randn(n) > 0
    assert a.dtype == numpy.dtype("bool")
    return a


def test_sort_bool_numpy(benchmark, ndarray_bool):
    benchmark.name = "numpy"
    benchmark(ndarray_bool.sort)

def test_sort_bool_pandas(benchmark, ndarray_bool, pandas):
    benchmark.name = "pandas"
    p = pandas.DataFrame(ndarray_bool)
    benchmark(lambda: p.sort_values(0))

def test_sort_bool_datatable(benchmark, ndarray_bool):
    benchmark.name = "datatable"
    d = datatable.DataTable(ndarray_bool)
    assert d.types == ("bool",)
    benchmark(lambda: d(sort=0))
